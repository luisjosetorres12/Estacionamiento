...
export class AddColombianHolidays1631031588426 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        for (let i = 0; i < diasLibres.length; i++) {
            ...
            await queryRunner.query(`insert into colombia_holidays(diaCelebracion,celebracion) values('${new Date(element.holiday).toISOString().split('T').join(' ').substr(0,MAX_LENGTH)}', '${element.celebration}')`, undefined);
        }
    }
